...
from _report_signals import (
    ...
    report_tokens as _report_tokens,
)
...
def _save_result(container, file_json, args,
    ...
    story = {
        ...
        "fact":     [],
        # Provenance stamp — auto-selector excludes prior st-fix outputs by default.
        # Underscore-prefixed to match _timestamp / _make convention. Old containers
        # (no _generated_by key) are treated as originals.
        "_generated_by": {
            "tool":      "st-fix",
            "mode":      args.mode,
            "source_s":  args.story,
...
Modes:
  ...
  synthesize   Merge all AI stories weighted by fact-check score
    ...
    parser.add_argument('--dry-run', action='store_true',
                        help='Show what would be changed without calling any AI or writing to the file')
    parser.add_argument('-v', '--verbose', action='store_true',
                        help='Show diff of each change')
    parser.add_argument('-q', '--quiet', action='store_true',
                        help='Minimal output')
    parser.add_argument('--include-fixed', action='store_true',
                        help='Include reports previously generated by st-fix in '
                             'auto-selection. Default: skip them so the originals '
                             'are fixed instead of "fixing the fix".')
    parser.add_argument('--weights', type=str, default=None,
                        help=argparse.SUPPRESS)   # power-user JSON override
    ...
    #
    # Selection is a *competition* between candidates judged on the likelihood
    # that st-fix can produce the best post-fix report.  Weighted formula
    # blends: claims count, score, false/partial mix, length-fit vs prompt
    # target, prompt-subject coverage, plus a default-AI tie-breaker and an
    # already-fixed penalty.  See st-internal/st-fix/README-st-fix.md.
    _W = {
        "claims":          1.0,
        "score":           2.0,
        "false":           0.5,
    ...
    if args.weights:
        try:
            _W.update(json.loads(args.weights))
        except (json.JSONDecodeError, TypeError) as e:
            print(f"Warning: could not parse --weights JSON: {e}; using defaults.")

    _signals      = _parse_prompt(_get_prompt_text(container))
    _default_ai   = get_default_ai()
    _AI_OPTS_DFLT = 1000   # AI-options "story" word default — used when prompt is silent

    def _score_candidate(s, fo, fixable_count):
        counts = fo.get("counts") or [0, 0, 0, 0, 0]
        n_partial = counts[3] if len(counts) > 3 else 0
        n_false   = counts[4] if len(counts) > 4 else 0
        score     = fo.get("score") or 0.0

        actual_words = len((s.get("text") or s.get("markdown") or "").split())
        target_mid   = _signals.target_words or _AI_OPTS_DFLT
        length_fit   = max(0.0, 1.0 - abs(actual_words - target_mid) / target_mid) \
                       if target_mid else 0.0

        rep_tokens = _report_tokens(s.get("text") or s.get("markdown") or "")
        cover = ((len(_signals.subjects & rep_tokens) / len(_signals.subjects))
                 if _signals.subjects else 0.0)

        is_default_ai = (s.get("make") == _default_ai)
        is_fixed      = bool(s.get("_generated_by"))

        p = (  _W["claims"]        * fixable_count
             + _W["score"]         * score
             + _W["false"]         * n_false
             + _W["partial"]       * n_partial
             + _W["length_fit"]    * length_fit
             + _W["subject_cover"] * cover
             + (_W["default_ai"]   if is_default_ai else 0.0)
             - (_W["fixed_penalty"] if is_fixed else 0.0))

        return {
            "priority":   round(p, 2),
            "length_fit": round(length_fit, 2),
            "cover":      round(cover, 2),
            "is_default": is_default_ai,
            "is_fixed":   is_fixed,
            "n_false":    n_false,
            "n_partial":  n_partial,
            "score":      score,
            "claims":     fixable_count,
        }

    def _why(row):
        """Return the 1–2 dominant scoring terms, e.g. 'topical coverage + many claims'."""
        terms = [
            ("many claims",          _W["claims"]        * row["claims"]),
            ("high score",           _W["score"]         * row["score"]),
            ("topical coverage",     _W["subject_cover"] * row["cover"]),
            ("length fit",           _W["length_fit"]    * row["length_fit"]),
            ("false-claim density",  _W["false"]         * row["n_false"]),
        ]
        terms.sort(key=lambda t: t[1], reverse=True)
        keep = [t[0] for t in terms[:2] if t[1] > 0.1]
        return " + ".join(keep) if keep else ""

    def _fixable_candidates(include_fixed=False):
        """Return list of dicts (one per (story, fact) pair with at least 1 fixable
        claim) ranked by weighted priority descending."""
        verifications = ["False", "Partially_false"]
        rows = []
        for si, s in enumerate(stories, 1):
            if not include_fixed and s.get("_generated_by"):
                continue
            for fi, f in enumerate(s.get("fact", []), 1):
                fc = extract_fact_checks(f, verifications, story=s)
                if fc:
                    m = _score_candidate(s, f, len(fc))
                    rows.append({"si": si, "fi": fi, "story": s, "fact": f, **m})
        rows.sort(key=lambda r: r["priority"], reverse=True)
        return rows

    def _print_fixable_table(candidates, highlight_s=None, highlight_f=None):
        """Print the ranked table including weighted priority + Why column."""
        print()
        print("  Fixable story/fact combinations (ranked by weighted priority):")
        print(f"  {'':2} {'S':>2}  {'F':>2}  {'Story AI':<11} "
              f"{'Score':>5} {'Claims':>6} {'False':>5} {'~Fls':>4} "
              f"{'Cover':>5} {'Fit':>4} {'Prio':>5}  Why")
        print("  " + "─" * 88)
        for r in candidates:
            sc_str = f"{r['score']:.2f}" if r['score'] is not None else " n/a"
            marker = " ◀" if r["si"] == highlight_s and r["fi"] == highlight_f else ""
            tag    = " *fix" if r["is_fixed"] else ""
            print(f"  {'':2} {r['si']:>2}  {r['fi']:>2}  "
                  f"{r['story'].get('make','?'):<11} "
                  f"{sc_str:>5} {r['claims']:>6} {r['n_false']:>5} "
                  f"{r['n_partial']:>4} {r['cover']:>5.2f} "
                  f"{r['length_fit']:>4.2f} {r['priority']:>5.2f}  "
                  f"{_why(r)}{tag}{marker}")
        print()
        if _signals.target_words:
            print(f"  Prompt target: ~{_signals.target_words} words"
                  f"  ·  subjects parsed: {len(_signals.subjects)}")
        else:
            print(f"  Prompt target: (none parsed; using default {_AI_OPTS_DFLT}w)"
                  f"  ·  subjects parsed: {len(_signals.subjects)}")
        if candidates:
            best = candidates[0]
            print(f"  Suggestion:  st-fix -s {best['si']} -f {best['fi']} "
    ...
    if auto_selected:
        candidates = _fixable_candidates(include_fixed=args.include_fixed)
        if not candidates:
            # Distinguish "all clean" from "no fact entries on any original".
            n_with_facts = sum(1 for s in stories if s.get("fact"))
            n_fixed      = sum(1 for s in stories if s.get("_generated_by"))
            n_originals  = n_stories - n_fixed

            if n_with_facts == 0:
                print(f"No fact entries found on any of the {n_stories} report(s) "
                      f"in this container.")
                print("If you previously fixed and then deleted a report, the fact")
            ...
            if not args.include_fixed and n_originals == 0 and n_fixed > 0:
                # All reports are st-fix outputs and they are all clean / excluded.
                print(f"  All {n_fixed} report(s) in this container were generated "
                      f"by st-fix and are excluded.")
                print(f"  To re-fix one of them:  "
                      f"st-fix --include-fixed {os.path.basename(file_json)}")
                sys.exit(0)

            if not args.include_fixed and n_fixed > 0:
                print(f"  No unfixed originals have remaining issues.")
                print(f"  {n_stories} report(s) present; {n_fixed} were generated "
                      f"by st-fix and are excluded.")
                print(f"  To re-fix one of those:  "
                      f"st-fix --include-fixed {os.path.basename(file_json)}")
                sys.exit(0)

            print("No False or Partially_false claims found in any fact-check.")
            print("Nothing to fix — all reports are clean.")
            sys.exit(0)
        ...
        if not args.quiet:
            print(f"  Auto-selected: story {args.story}, "
                  f"fact-check {args.fact}  "
                  f"(priority {best['priority']:.2f}, {best['claims']} claims to fix)")
    ...
    if args.mode == "synthesize":
        ...
        if args.dry_run:
            print()
            print("  Dry run — no AI calls made, no file written.")
            print(f"  To run for real:  st-fix --mode synthesize {os.path.basename(file_json)}")
            return
